...
function capitalizeFirstLetter(myString) {
  myString = myString.replace(/\b(\w)/g, function (w) {
...
function handleMouseOver(d) {
  //d3 bar chart on mouse over event

  d3.select(this).transition().style("fill", "#fffccc").duration(300);
...
function handleMouseOut(d) {
  //d3 bar chart on mouseout event
  d3.select(this)
    .transition()
...
function drawBar(data) {
  ...
  let listOfValues = data
    .map((d) => d["average total income"])
    .concat(data.map((a) => a["average total expenditure"]));
  ...
  barSvg.append("g").call(yAxis); // text label for y-axis
  barSvg
    .append("text")
    .text("$('000)")
    .attr("x", -150)
  ...
  barSvg
    ...
    .attr("transform", function (d) {
      return "translate(" + xScale(d.quintile) + ",0)";
    })
    .selectAll("rect")
    .data(function (d) {
...
function renderPieRow(
  ...
  let rowWidth =
    $("#horizontalChart").width() - rowMargin.right - rowMargin.left;
  ...
  PieChart.width(pieWidth)
    ...
    .innerRadius(25)
    .colors(
      d3.scaleOrdinal(d3.quantize(d3.interpolateHcl("#68ab77", "#d8485c"), 2))
    )
    .title(function (d) {
      d.value = parseFloat(d.value.toFixed(2));
      return d.key + ": " + d.value;
    ...
    chart.on("filtered", function () {
      ...
      if (chart.filters()[0] == "user" && chart._group.top(2)[1].value > 0) {
        rowColor = d3.scaleOrdinal(
          d3.quantize(d3.interpolateHcl("#7e1b28", "#e4818e"), 9)
        );
        RowChart.colors(rowColor);
      } else if (
        chart.filters()[0] == "national" &&
        chart._group.top(2)[1].value == 0
      ) {
        //not user
        rowColor = d3.scaleOrdinal(
...
function renderSeries(chartSeries, dimension, group, earlierDate, currentDate) {
  ...
  let lineWidth =
    $("#series-chart").width() - lineMargin.left - lineMargin.right;
  ...
  chartSeries
    ...
    .height(lineHeight)
    .chart(function (c) {
      return new dc.LineChart(c).curve(d3.curveLinear);
    })
  ...
  chartSeries.yAxis().tickFormat(function (d) {
    return d3.format(",d")(d);
  });
  chartSeries.xAxis().tickFormat(function (d) {
...
function chart_display(rowChart, pieChart, seriesChart) {
  ...
  rowChart.svg().selectAll("svg").remove();
  pieChart.svg().selectAll("svg").remove();
  seriesChart.svg().selectAll("svg").remove();
